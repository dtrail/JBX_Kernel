...
static void hotplug_suspend(int suspend)
{
	if (!susp_enabled) return;
		...
		pr_info("[dtrail] hotplugjbx awake cpu1 up\n");
	} else {
		suspended = 1;
                if (num_online_cpus() > 1) cpu_down(1);
		pr_info("[dtrail] hotplugjbx suspended cpu1 down\n");
	}
}
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		registration = 0;
                pr_info("[dtrail] hotplugjbx start\n");
		break;
	...
	case CPUFREQ_GOV_STOP:
		 ...
		 */
	        susp_enabled = 0;
                ...
                pr_info("[dtrail] hotplugjbx inactive\n");
		break;
...
{
	SYMSEARCH_BIND_FUNCTION_TO(hotplugjbx, cpu_up, cpu_up_s);
	int err;
	cputime64_t wall;
	u64 idle_time;
	int cpu = get_cpu();
...
MODULE_DESCRIPTION("'cpufreq_hotplugJBX' - Hotplug governor + early suspend support & tweaks\nextra : Moto**** ported version.");
MODULE_LICENSE("GPL");
